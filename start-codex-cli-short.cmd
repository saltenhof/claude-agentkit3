...
start "Codex CLI [short]" "%PS%" -NoExit -Command ^
  "$env:LANG='en_US.UTF-8'; $env:LC_ALL='en_US.UTF-8'; $env:LESSCHARSET='utf-8'; $env:PYTHONUTF8='1'; Set-Location -LiteralPath '%WORKDIR%'; Write-Host ''; Write-Host ' Codex CLI - Short Context' -ForegroundColor Cyan; Write-Host ''; & '%CODEX_EXE%' -c '%MCP_COMMAND_CONFIG%' -c '%MCP_ARGS_CONFIG%' -c '%MCP_CWD_CONFIG%' -c model_context_window=%CONTEXT_WINDOW% -c model_auto_compact_token_limit=%AUTO_COMPACT% -C '%WORKDIR%'"
