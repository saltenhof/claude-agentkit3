        ...
        facts: [
          {
            ...
            opened_at: fiveMinAgo,
            closed_at: fiveMinAgo,
            qa_round_count: 4,
            adversarial_findings_count: 1,
            are_gate_passed: true,
            computed_at: fiveMinAgo,
          },
        ],
      }),
    });
  ...
  it('AG3-116-KPI-DTO-3: GET /kpi/guards responds with FK-62-named guard_key (no guard_id)', async () => {
    ...
    const seedResp = await fetch(`${baseUrl}/_test/seed-kpi-facts`, {
      ...
      body: JSON.stringify({
        ...
        guards: [
          {
            guard_key: 'ag3116_test_guard',
            period_start: periodStart,
            invocation_count: 7,
            violation_count: 1,
            computed_at: periodEnd,
          },
        ],
      }),
    });
        ...
        pipeline: [
          {
            period_start: periodStart,
            story_count: 14,
            story_count_closed: 12,
            qa_round_avg: 3.5,
            processing_time_avg_ms: 90000,
            computed_at: periodEnd,
          },
        ],
      }),
    ...
    const seedResp = await fetch(`${baseUrl}/_test/seed-kpi-facts`, {
      ...
      body: JSON.stringify({
        ...
        corpus: [
          {
            period_start: periodStart,
            new_incident_count: 5,
            patterns_total_count: 8,
            patterns_with_active_check: 6,
            computed_at: periodEnd,
          },
        ],
      }),
