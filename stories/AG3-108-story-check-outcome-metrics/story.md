...
### 2.1 In Scope
1. **FK-69-Konzept-Eintrag (Approval-Flow, ZUERST):** in FK-69 ein **Per-Check-Outcome-Read-Model** definieren (z. B. `qa_check_outcomes`), **Owner verify-system**: Key `(project_key, run_id, stage_id, attempt_no, check_id)` (+ `story_id`) — `stage_id`/`attempt_no` sind **Pflicht-Identitaetsfelder**, da derselbe `check_id` ueber QA-Remediation-Attempts/Stages mehrfach im selben Run laufen kann; Outcome-Enum **triggered | clean | overridden**, `occurred_at` (UTC), optional `check_proposal_ref` (FK auf `fc_check_proposals.check_id` **nur**, wenn der Check tatsaechlich aus einem Proposal stammt), Override-Korrelation. Ablauf strikt: **Codex absegnen (write=false) → Edit `concept/…` → Codex re-review → GAC-1/Concept-Gates grün.** Erst danach Code.
