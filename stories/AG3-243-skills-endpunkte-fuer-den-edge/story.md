# AG3-243 — Die Skill-Bindungsabfrage ist der Weg, ueber den der Hook-Prozess heute den gesamten Kern nachzieht — 16 Symbole, und keines davon gehoert auf den Laptop.

- **Typ:** implementation
- **Groesse:** M
- **Bounded Context:** `agent-skills`
- **Betroffene Subpakete:** `backend/skills/` sowie `state_backend/store/skill_binding_repository`
- **BC-Konzepte:** DK-01, DK-12, FK-43
...
Dieser BC traegt den Ersatz fuer den Sicherheitsdefekt, den AG3-239 benennt und
AG3-209 aufloest: `governance/hook_event_inputs.py:46` ruft lazy
`bootstrap.composition_root.build_skills`, um zu pruefen, ob eine Skill-Bindung
existiert — und **zieht damit den gesamten Kern in den Hook-Prozess**.

Die Abfrage selbst ist berechtigt. Der Weg ist es nicht. Solange es keine
`/v1`-Operation „ist diese Skill fuer dieses Projekt gebunden" gibt, hat der
Hook keine Alternative zum Composition-Root — und **genau deshalb ist diese
Story eine Vorbedingung von AG3-209**, nicht eine Nacharbeit.
...
| Groesse | Startwert |
|---|---|
| `skills` — Verhalten + ungeklaert (Vormessung) | 1 + 11 = **12** |
| Grenzverletzungspaare Edge→Kern in `skills` | **16** |
| davon eigene Messung: distinkte Symbole | **16** |
| `state_backend/store/skill_binding_repository` | **1** (`StateBackendSkillBindingRepository`) |
...
Elf der zwoelf Posten stehen in der Vormessung als **ungeklaert** — der hoechste
Anteil aller BCs. Der Grund ist mechanisch und in AC 1 aufzuloesen:
`backend/skills/__init__.py` re-exportiert fast alles (`Skills`,
`SkillBinding`, `SkillBundleStore`, `SkillProfile`, `assess_bundle_version`,
`is_directory_link`, `remove_directory_link`), und der Klassifikator der
Vormessung konnte nicht entscheiden, was dahinter liegt. **Hier ist die eigene
Messung nicht Bestaetigung, sondern Erstmessung.**

Die Trennlinie duerfte zwischen zwei Gruppen verlaufen:

- **Bindungswissen** (`SkillBinding`, `SkillBindingRepository`,
  `SkillBindingMode`, `SkillLifecycleStatus`, `assess_bundle_version`) — Kern.
- **Dateisystemarbeit am Projekt** (`is_directory_link`,
  `remove_directory_link`, `materialize.bundle_has_placeholders`) — laeuft
  zwangslaeufig dort, wo das Projekt liegt.

## Scope

### In Scope

- Fuer jede der 16 Ueberquerungen die Antwort (a)/(b)/(c) nach AC 1, inklusive der Aufloesung der elf Re-Export-Faelle.
- **Die `/v1`-Operation fuer die Skill-Bindungsabfrage des Hooks.** Sie ist der benannte Ersatz fuer `composition_root.build_skills` in `hook_event_inputs.py:46` und muss ohne Kern-Import auskommen. Ihre Latenz liegt im synchronen Hook-Pfad — das ist eine Entwurfsbedingung, kein Detail.
- Der fail-closed Ausgang, wenn der Kern die Bindung nicht beantworten kann. Heute schluckt `hook_event_inputs.py:50` jede Exception und meldet `False` — das ist ein stiller Fallback auf schlechtere Datenqualitaet (`CLAUDE.md` §NO ERROR BYPASSING) und darf nicht in den Endpunkt-Client wandern.

### Out of Scope

- **Das Wandern von `hook_event_inputs.py` selbst** — **Owner: AG3-209**
  (Eingangsliste E6). Diese Story liefert die Operation, die das Wandern
  moeglich macht.
- **AG3-225 (`skill-bundles-verlassen-den-menschenpfad`)** bleibt unberuehrt.
