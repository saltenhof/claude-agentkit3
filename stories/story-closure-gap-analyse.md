...
|---|---|---|---|
| A1 | Finding-Resolution-Gate (ClosureGates-Sub) | `FK-29 §29.2`, `concept/_meta/bc-cut-decisions.md §BC7` | Kein Code unter `agentkit.closure.gates`. Gate-Pruefung gegen Layer-2-Artefakte (qa_review/semantic_review/doc_fidelity) fehlt vollstaendig; ClosurePhaseHandler.on_enter fuehrt diesen Schritt nicht aus. |
| A2 | Integrity-Gate-Delegation an governance-and-guards | `FK-29 §29.1.2`, `FK-35 §35.2`, `concept/_meta/bc-cut-decisions.md §BC7` | `agentkit.closure.gates` und damit der `IntegrityGateInvoker` fehlen. Kein Code, der `agentkit.governance.integrity_gate` vor dem Merge aufruft. |
| A3 | MergeSequence-Sub (Branch-Push, Merge, Worktree-Teardown als recovery-idempotente Substates in Single-Repo) | `FK-29 §29.1.2`, `FK-29 §29.1.3`, `concept/_meta/bc-cut-decisions.md §BC7` | `agentkit.closure.merge_sequence` existiert nicht. Single-Repo-Branch-Push und -Merge fehlen; `multi_repo_saga.py` deckt nur den Multi-Repo-Pfad ab. `ClosurePhaseHandler.on_enter` delegiert nicht an eine MergeSequence. |
...
| A5 | Postflight-Gates (fuenf Checks: story_dir_exists, story_closed, metrics_set, telemetry_complete, artifacts_complete) | `FK-29 §29.3`, `concept/_meta/bc-cut-decisions.md §BC7 PostMergeFinalization` | Kein Code unter `agentkit.closure.post_merge_finalization` jenseits des Records-Moduls. Keine Klasse `PostflightGate` oder `PostflightCheck`. |
| A6 | Rückkopplungstreue Ebene 4 (Dokumententreue via LlmEvaluator) | `FK-29 §29.1.4 Schritt 8`, `FK-38 §38.3.1`, `concept/_meta/bc-cut-decisions.md §BC7 PostMergeFinalization` | `FeedbackFidelityCheck` fehlt. Keine Verbindung zu `agentkit.verify_system.llm_evaluator` aus dem Closure-Pfad. |
...
| A8 | Guard-Deaktivierung via Governance.deactivate_locks(story_id) | `FK-29 §29.5`, `concept/_meta/bc-cut-decisions.md §BC7 PostMergeFinalization` | `GuardDeactivator` fehlt. Kein Aufruf an `agentkit.governance` zum Beenden des Lock-Records und Entfernen der Lock-Exporte. |
| A9 | ClosureVerdict (COMPLETED/ESCALATED) und MergePolicy (ff_only/no_ff) als typisierte Enums im BC | `FK-29 §29.1.5`, `concept/_meta/bc-cut-decisions.md §BC7 Top-Datenmodell` | Weder `ClosureVerdict` noch `MergePolicy` sind als Typen im `agentkit.closure`-Namespace definiert. Die Konzept-Skizze erwartet diese als Teil des Top-Datenmodells. |
...
| C2 | Closures Sub-Komponenten-Paketschnitt weicht von bc-cut-decisions ab | `src/agentkit/closure/` (Verzeichnisstruktur) | `concept/_meta/bc-cut-decisions.md §BC7 Modul-Prefixes` | BC-cut-decisions erwartet `agentkit.closure.gates`, `agentkit.closure.merge_sequence`, `agentkit.closure.post_merge_finalization`, `agentkit.closure.execution_report`. Tatsaechlich existieren nur `agentkit.closure.execution_report` (fast leer) und `agentkit.closure.post_merge_finalization` (nur Records). `agentkit.closure.gates` und `agentkit.closure.merge_sequence` fehlen; die Haupt-Phase-Logik liegt stattdessen in `agentkit.pipeline.phases.closure` — ein sachfremder Schnitt, der dem Top-Surface-Prinzip des BC widerspricht. |
...
4. **BC-Paketschnitt gemaess bc-cut-decisions korrigieren (C2).** Phase-Logik gehoert in `agentkit.closure.*`-Sub-Module, nicht in `agentkit.pipeline.phases.closure`. Der aktuelle Schnitt verletzt das Top-Surface-Prinzip und erschwert Erweiterungen.
