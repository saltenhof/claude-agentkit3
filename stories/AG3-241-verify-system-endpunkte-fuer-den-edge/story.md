# AG3-241 — Evidence-Assembly und die LLM-Bewertungsstrecke laufen heute im Edge-Prozess — der QA-Subflow des Kerns ist vom Laptop aus direkt aufrufbar.

- **Typ:** implementation
- **Groesse:** M
- **Bounded Context:** `verify-system`
- **Betroffene Subpakete:** `backend/verify_system/`
- **BC-Konzepte:** DK-04, DK-11, FK-27, FK-28, FK-32, FK-33, FK-34, FK-37, FK-38, FK-46, FK-47, FK-48
...
`verify-system` ist ein **Capability-Bounded-Context** und laeuft im Kern: die
4-Schichten-QA, die Policy Engine und die LLM-Bewertungen sind genau das, was
nicht auf dem Laptop des Entwicklers entschieden werden darf — sonst kann der
Worker seine eigene Bewertung beeinflussen (`CLAUDE.md`
§WORKFLOW- UND STATE-DISZIPLIN: „Worker duerfen ihre eigenen QA-Ergebnisse nicht
manipulieren").

Heute holt der Edge `EvidenceAssembler`, `StructuredEvaluator`, `HubLlmClient`
und sogar `_PromptMaterializer` als Python-Symbole. Damit laeuft
Bewertungsmaschinerie im selben Prozess wie der Bewertete. Das ist nicht nur ein
...
| Groesse | Startwert |
|---|---|
| `verify_system` — Verhalten + ungeklaert (Vormessung) | 3 + 7 = **10** |
| Grenzverletzungspaare Edge→Kern in `verify_system` | **19** |
| davon eigene Messung: distinkte Symbole | **20** |
...
Die 20 Symbole in drei Gruppen:

- **Evidence-Sammlung** (`evidence.EvidenceAssembler`, `ImportResolver`,
  `RepoContext`, `VerifyEvidenceFile`, `structural.system_evidence.ChangeEvidence`) —
  die Rohdaten entstehen zwangslaeufig auf dem Laptop, die Bewertung nicht. Hier
  ist die Grenze fachlich interessant und AC 1 muss sie ziehen.
- **LLM-Bewertung** (`llm_evaluator.*`, 10 Symbole inklusive des privaten
  `_PromptMaterializer`) — Kern. Ein Edge, der `HubLlmClient` selbst haelt, hat
  den Bewertungspfad in der Hand.
- **Vokabular** (`protocols.Finding`, `Severity`, `TrustClass`) — Daten,
  **Owner AG3-237**.

`core_types.verify_evidence` (11 Symbole) traegt denselben Vorgang, liegt aber
in `core_types` und ist Daten — ebenfalls AG3-237. Diese Story baut den
**Vorgang**, nicht die Nutzlast.

## Scope

### In Scope

- Fuer jede der 19 Ueberquerungen die Antwort (a)/(b)/(c) nach AC 1.
- Die Evidence-Uebergabe wird EINE Operation: der Edge liefert die gesammelte Evidenz, der Kern bewertet. Nicht: der Edge ruft Assembler, Resolver und Evaluator nacheinander ueber das Netz — AC 3.
- Der Nachweis, dass nach dem Schnitt aus dem Edge-Prozess **kein** Symbol der LLM-Bewertungsstrecke mehr erreichbar ist. Das ist der eigentliche Gewinn dieser Story und gehoert als Negativtest belegt.

### Out of Scope

- **Die 4-Schichten-QA selbst, Stage-Registry, Policy Engine und
  Trust-Klassen** — unveraendert. Diese Story verlegt den Aufrufweg, sie aendert
  keine Bewertungsregel.
- **`core_types.verify_evidence`** — Daten, **Owner AG3-237**.
