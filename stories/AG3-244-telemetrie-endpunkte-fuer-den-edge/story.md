# AG3-244 — Der Edge emittiert Telemetrie heute teils ueber REST und teils ueber direkt importierte Kern-Emitter — zwei Wege fuer denselben Vorgang.

- **Typ:** implementation
- **Groesse:** M
- **Bounded Context:** `telemetry-and-events`
- **Betroffene Subpakete:** `backend/telemetry/` sowie die Telemetrie-Leseseite in `state_backend/store/`
- **BC-Konzepte:** DK-05, FK-68, FK-69
...
Telemetrie ist der eine Bereich, in dem der REST-Weg **bereits existiert**:
AG3-129 hat Guard-Counter, Worker-Health und Telemetrie ueber die
ProjectEdge-Strecke vermittelt, und `telemetry/rest_emitter.py` benutzt ihn.
Daneben importiert der Edge aber weiterhin `telemetry.storage.StateBackendEmitter`,
`telemetry.hooks.*` und `telemetry.audit_bundle.AuditBundleExporter` direkt.

Damit gibt es zwei Wege fuer denselben Vorgang — genau der Zustand, den
`CLAUDE.md` §SINGLE SOURCE OF TRUTH ausschliesst. Diese Story macht den
REST-Weg zum einzigen.
...
| Groesse | Startwert |
|---|---|
| `telemetry` — Verhalten + ungeklaert (Vormessung) | 7 + 0 = **7** |
| Grenzverletzungspaare Edge→Kern in `telemetry` | **15** |
| davon eigene Messung: distinkte Symbole | **12** |
| Telemetrie-Leseseite in `state_backend/store/` | **2** (`telemetry_read_repository`, `telemetry_projection_repository_misc`) |
...
- **Schreibweg** (`storage.StateBackendEmitter`, `emitters.EventEmitter`,
  `emitters.NullEmitter`) — der `StateBackendEmitter` schreibt direkt in die
  Datenbank des Kerns und ist damit der klarste Fall fuer (c) beziehungsweise
  einen Ersatz durch den vorhandenen REST-Emitter.
- **Hook-seitige Emitter** (`hooks.base.HookContext`, `HookTrigger`,
  `budget_event_emitter.BudgetEventEmitter`, `commit_hook.CommitHook`,
  `review_guard.ReviewGuard`) — laufen im Hook-Prozess; erwartete Antwort (b).
- **Audit-Bundle** (`audit_bundle.AuditBundleExporter`,
  `AuditBundleExportError`) — Kern.
- **Vokabular** (`events.Event`, `events.EventType`) — Daten, **Owner
  AG3-237**.
...
- Fuer jede der 15 Ueberquerungen die Antwort (a)/(b)/(c) nach AC 1.
- **Genau ein Schreibweg fuer Telemetrie.** Der REST-Weg aus AG3-129 bleibt; der Direktimport von `StateBackendEmitter` entfaellt. Kein zweiter Transport, kein Direkt-DB-Fallback.
- Die Telemetrie-Leseseite (Event-Abfrage, Phase-State-Projektion) wird eine Leseoperation, nicht drei Repository-Aufrufe — AC 3.

### Out of Scope

- **`telemetry.events.{Event, EventType}`** — Daten, **Owner AG3-237**.
- **Der AG3-129-REST-Transport selbst** — existiert, bleibt, wird nicht ersetzt.
- **Die KPI-Aggregation auf denselben Daten** — **Owner: AG3-245**
  (kpi-and-dashboard). Die Grenze verlaeuft an der Faktentabelle: Ereignisse
  gehoeren hierher, Fakten und Aggregate dorthin.
