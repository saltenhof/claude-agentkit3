...
- **Die zwei ausgelieferten Bundles tragen noch ~30 Board-Tokens + 12× `{{GH_CONFIG_EXPORT}}`:**
  - `src/agentkit/resources/skill_bundles/create-userstory-core/4.0.0/SKILL.md` — „Step 6: Add to GitHub Project and Set Custom Fields" (`:1204-1375`): `gh project item-add` (`:1210`), `gh project item-list` (`:1217`), 12× `gh api graphql mutation updateProjectV2ItemFieldValue` (`:1250-1372`) mit `{{GH_PROJECT_ID}}`/`{{GH_FIELD_*}}`/`{{GH_*_OPTION}}`; „Step 4: Determine Next Story ID" (`:528-557`) liest die ID angeblich „from the GitHub Project custom field"; Status-Setzen „via GitHub Project Board UI or GraphQL CLI" (`:1429-1430`).
...
3. **`create-userstory-core/4.0.0/SKILL.md` Verhaltens-Re-Cut (D3):**
   - **Step 6 (Board)** vollstaendig entfernen: kein `gh project item-add/item-list`, keine `gh api graphql` Field-Mutations; Story-Anlage/-Attribute laufen ueber das **deployte AG3-114-Tool** `projectedge create-story` (das `ProjectEdgeClient.create_story` gegen die tenant-scoped `POST /v1/projects/{project_key}/stories` fuehrt — fail-closed Reconciliation inkl. LLM-Adjudicator, atomic id-Allokation). Die Skill-Schritte beschreiben **diesen** Tool-Aufruf, nicht in-process `StoryService`-Aufrufe (Agents gehen ueber den Client, §91.1a Regel #3).
   - **Step 4 Next-ID entfaellt als eigener Schritt:** die Story-ID wird vom Backend **atomar in `create_story`** alloziert und zurueckgegeben — **kein** Standalone-`next_story_id`-Prefetch, **kein** `agentkit story validate` (existiert nicht als Subcommand; die Validierung passiert non-bypassable in der Create-Boundary, AG3-068). Der stale „reads from GitHub Project"-Kommentar entfaellt mit dem Schritt.
   - **`gh issue create`, `gh issue list`/`gh issue view`, `{{GH_CONFIG_EXPORT}}`** entfernen (D3): **jegliche** `gh issue`-Story-Tracking-Nutzung verschwindet (Anlage, Related-Suche, Status), nicht nur `gh issue create`; Anlage laeuft ueber das AG3-114-Tool, Index/Wiki weiter ueber `export-story-md`. Kein gh-Auth-Prelude mehr.
...
3. **create-userstory-core board-frei + AK3-nativ ueber das AG3-114-Tool verdrahtet:** Im re-cut `SKILL.md` gibt es **kein** `gh project`, **kein** `gh api graphql` (Board), **kein** `gh issue create`, **kein** `gh issue list` (Related-Story-Suche), **kein** `{{GH_CONFIG_EXPORT}}`, **keinen** `{{GH_FIELD_*}}`/`{{GH_*_OPTION}}`/`{{GH_PROJECT_*}}`-Token. **Story-Anlage laeuft ueber das deployte AG3-114-Tool** `projectedge create-story` (das den `ProjectEdgeClient` gegen die tenant-scoped `POST /v1/projects/{project_key}/stories` fuehrt, fail-closed Reconciliation inkl. LLM-Adjudicator). **Die Story-ID wird vom Backend ATOMAR in `create_story` alloziert** — **kein** Standalone-`next_story_id`-Prefetch, **kein** `agentkit story validate` (existiert nicht als Subcommand; Validierung passiert in der Create-Boundary). Related-Story-Suche laeuft ueber den Story-Read-Service (FK-91) / VectorDB-Reconciliation, nicht `gh issue list`. Test/Assertion: Inhaltsscan (kein `gh project`/`gh api graphql`/`gh issue`/Board-Token) + Verweis auf das AG3-114-`create-story`-Tool.
4. **lookup-userstory-core board-frei + AK3-verdrahtet:** keine „Source B"-Board-Query, kein `gh project item-list`, **kein `gh issue view`/`gh issue list`**, keine Board-Feld-Matches; Lookup ueber den Story-Read-Service (`StoryService.list_stories/get_story` bzw. die §91.1a-Read-Endpoints `/v1/stories`, `/v1/stories/{id}`); `{{REPO_LAYOUT_TABLE}}` aus `config.repositories[]`. Test/Assertion: Inhaltsscan (keinerlei `gh issue`/`gh project`).
...
7. **Toter Adapter entsorgt:** `integrations/github/projects.py` existiert nicht mehr; der Re-Export ist aus `integrations/github/__init__.py` entfernt; `import agentkit.integration_clients.github` und ein Grep ueber `src/` belegen **keinen** lebenden `projects`-Konsumenten; Suite gruen ohne die entfernten Selbsttests.
...
8a. **execute-userstory-core Vokabular mitgezogen (AG3-111-E2E-Ziel):** das `execute-userstory-core`-Bundle (AG3-111s reale AC4-Materialisierung) traegt nach diesem Re-Cut ausschliesslich aufloesbare, **lowercase** Tokens (kein UPPERCASE-`{{PROJECT_PREFIX}}`; `{{USERSTORY_BUNDLE_PATH}}` lowercase + als ueberlebender Layout-Token mit Quelle ODER derived/inline; `{{AGENT_SPAWN_SKILL_PROOF}}` unveraendert AG3-110). Kein Verhaltens-Re-Cut (keine Board-Schritte), aber Vokabular-Konformitaet ist Pflicht, sonst bleibt AG3-111 blockiert. Test: `substitute_spawn_header(...)` ueber `execute-userstory-core` ohne `UnknownPlaceholderError`, kein residualer `{{...}}` (ausser `<STORY-ID>`/`<ROUND>`/Block-Direktiven).
...
- **Reihenfolge ist verbindlich:** (0) **read-only Design-Pass ZUERST** — bestimme empirisch aus dem geplanten Re-Cut (Inlining/Derive) die **tatsaechlich ueberlebenden** Tokens, BEVOR du das Konzept editierst (sonst ist der Approval-Flow nicht erzwingbar — du wuesstest nicht, welche FK-03/`ProjectConfig`-Felder noetig sind). Dann (1) Konzept-Delta (FK-43/FK-03 + `ProjectConfig`) via Codex-Approval-Flow, (2) Substitutor-Vokabular, (3) Bundle-Re-Cut (create via AG3-114-`create-story`-Tool, lookup via Read-Service), (4) Vokabular-Verifikation (inkl. execute-userstory-core), (5) Dead-Code (`projects.py`). Substituiere nie gegen ein Bundle, dessen Tokens das Konzept/der Substitutor noch nicht kennt.
- **Create-Anlage ueber das AG3-114-Tool, nicht neu bauen:** der re-cut create-userstory-Skill ruft das deployte `projectedge create-story`-Tool (AG3-114) — `ProjectEdgeClient.create_story` gegen `/v1/projects/{project_key}/stories`, atomic id-Allokation, fail-closed Reconciliation. Kein `gh issue create`/`gh issue list`, kein Standalone-`next_story_id`, kein `agentkit story validate`.
