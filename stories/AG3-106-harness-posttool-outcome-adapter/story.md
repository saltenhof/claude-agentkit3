...
- **Falls Claude fehlgeschlagene Befehle als `PostToolUseFailure` liefert:** `HookEventName` (`governance/hook_registration.py:24`) + DB-CHECK in **beiden** Stores (`state_backend/sqlite_store.py:465` **und** `state_backend/postgres_schema.sql:216`, `store/governance_hook_repository.py`) lassen heute nur `PreToolUse`/`PostToolUse` zu — die Aufnahme von `PostToolUseFailure` (Enum + CHECK-Constraint in beiden Stores) ist dann Teil dieses Scopes.
