...
- **Post-Merge-Finalization-SCHRITTE sind IN Scope** (FK-29 §29.1.4 Schritte 6-9; BC 7 `PostMergeFinalization`): Rueckkopplungstreue Ebene 4 (FK-38 §38.3.1), Postflight-Gates (5 Checks, FK-29 §29.3), VektorDB-Sync (§29.1.4 Schritt 8), Guard-Deaktivierung (FK-29 §29.5) werden alle als **verbindliche, non-blocking Pflichtschritte verdrahtet** (siehe 2.1.1 Schritt 6-9). **Out-of-Scope ist nur die fachliche TIEFE einzelner Bestandteile**, nicht das Auslassen der Schritte:
  ...
  - **ExecutionReport-9-Sektionen-Ausbau (FK-29 §29.4)** — separater Owner (`agentkit.backend.closure.execution_report`); der bestehende flache Report bleibt; der Report-Aufruf am Closure-Ende bleibt unveraendert.
...
- **NICHT neu bauen**: Saga (`agentkit.backend.closure.multi_repo_saga`: `run_multi_repo_closure`, `pre_merge_check`, `local_ff_merge_with_rollback`, `push_main`, `push_story_branches`, `teardown_worktrees`, `GitBackend`-Protokoll), IntegrityGate (`agentkit.backend.bootstrap.composition_root.build_integrity_gate` -> `IntegrityGate.evaluate`), Sonar-Gate (`agentkit.backend.verify_system.sonarqube_gate`: `build_sonar_gate_port_for_run`, `evaluate_sonarqube_gate`). Diese Story ruft sie auf.
