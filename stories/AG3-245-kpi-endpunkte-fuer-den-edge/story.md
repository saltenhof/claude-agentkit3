# AG3-245 — Der Edge baut sich KpiAnalytics, DashboardService und den FactStore lokal zusammen — fuenf Symbole, die alle in einer Datenbank enden, die er nicht sehen darf.

- **Typ:** implementation
- **Groesse:** S
- **Bounded Context:** `kpi-and-dashboard`
- **Betroffene Subpakete:** `backend/kpi_analytics/` sowie die Faktenseite in `state_backend/store/`
- **BC-Konzepte:** DK-13, FK-60, FK-61, FK-62, FK-63, FK-64
...
Dieser BC ist klein und eindeutig: **jedes** der betroffenen Symbole endet in
der Postgres-Instanz des Kerns. `FactStore`, `StateBackendFactRepository`,
`StateBackendAnalyticsSource` und `RefreshWorker` sind Persistenz- und
Aggregationsmaschinerie; `DashboardService` und `KpiCatalog` sind
Leseoberflaechen darueber.

Es gibt keinen Grund, warum davon irgendetwas auf dem Laptop laufen sollte —
und genau deshalb ist die Story klein: die erwartete Antwort ist fast durchgaengig
(a), und die Frage lautet nur, **wie viele** Operationen es werden.
...
| Groesse | Startwert |
|---|---|
| `kpi_analytics` — Verhalten + ungeklaert (Vormessung) | 0 + 5 = **5** |
| Grenzverletzungspaare Edge→Kern in `kpi_analytics` | **4** |
| davon eigene Messung: distinkte Symbole | **5** (`KpiAnalytics`, `KpiCatalog`, `aggregation.RefreshWorker`, `dashboard.DashboardService`, `fact_store.FactStore`) |
| Faktenseite in `state_backend/store/` | **2** (`fact_repository`, `analytics_source`) |
...
Alle fuenf Posten stehen in der Vormessung als **ungeklaert**, weil
`backend/kpi_analytics/__init__.py` sie re-exportiert. Die eigene Messung ist
hier also Erstmessung, aber der Umfang ist ueberschaubar.

**AC 3 ist der eigentliche Inhalt dieser Story.** Ein Dashboard, das seine
Kacheln einzeln abfragt, erzeugt aus fuenf Symbolen fuenfzehn Endpunkte. Die
richtige Antwort ist eine Leseoperation pro **Sicht**, nicht pro Kennzahl.

## Scope

### In Scope

- Fuer jede der 4 Ueberquerungen die Antwort (a)/(b)/(c) nach AC 1.
- Die KPI-Leseseite wird ueber `/v1` erreichbar, zugeschnitten auf die **Sichten**, die der Edge tatsaechlich anzeigt — AC 3.
- `RefreshWorker` und `FactStore` laufen nachweislich nur noch im Kern.

### Out of Scope

- **Der KPI-Katalog selbst, Erhebungsregeln und Aggregationslogik** —
  unveraendert. Diese Story verlegt den Zugriffsweg.
- **Das Frontend-BFF und die Dashboard-Oberflaeche** (FK-72) — eigener
  Schnitt, nicht Gegenstand des Distributionsproblems.
- **Die Ereignisseite derselben Daten** — **Owner: AG3-244**.
