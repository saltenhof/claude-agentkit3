...
from agentkit.story.repository import StoryRepository as _StoryRepository

if TYPE_CHECKING:
    from collections.abc import Callable

    from agentkit.execution_planning.repository import ParallelizationConfigRepository
    ...
    from agentkit.requirements_coverage import AreClient
...
class ReadModelRoutes:
    ...
    Args:
        ...
        phase_state_loader: Callable that resolves the current
            :class:`~agentkit.pipeline_engine.phase_executor.PhaseState` for
            a story by story_id.  Defaults to
            ``agentkit.story.repository.load_phase_state_global`` (production
    ...
    def _load_current_phase_state(
        ...
        ``agentkit.story.repository.load_phase_state_global``) to read the
    ...
    def _resolve_are_client(self, project_key: str) -> AreClient | None:
        ...
        if not are_url:
            return None
        from agentkit.requirements_coverage import AreClient as _AreClient
