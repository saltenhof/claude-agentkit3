...
the AG3-088 :class:`~agentkit.installer.checkpoint_engine.engine.CheckpointEngine`
...
* :class:`UpgradeRunState` — the mutable per-run state the handlers fill (the
  ...
  installer :class:`~agentkit.installer.checkpoint_engine.context.CheckpointRunState`
...
from agentkit.installer.upgrade.cleanup import run_cleanup
from agentkit.installer.upgrade.config_migration import migrate_config_file
...
from agentkit.installer.upgrade.scenarios import decide_upgrade_scenario
...
if TYPE_CHECKING:
    ...
    from agentkit.installer.upgrade.cleanup import CleanupOutcome, CleanupPlan
    from agentkit.installer.upgrade.hook_migration import (
        GitHookMigrationOutcome,
        HookMigrationOutcome,
    )
    from agentkit.installer.upgrade.scenarios import UpgradeScenarioDecision
...
def up_03_migrate_config(context: UpgradeRunContext) -> CheckpointResult:
    ...
    """
    from agentkit.installer.paths import project_config_path
    from agentkit.installer.upgrade.config_migration import read_config_version
