...
``protected_module_prefixes``: ``agentkit.backend.governance`` / ``agentkit.pipeline``
/ ``agentkit.qa.structural`` may not contain these strings literally).

Callers:
- ``agentkit.backend.state_backend`` (persistence, projection write path) — may
  use the strings productively (state_backend is a T-driver, not
  protected).
- ``agentkit.backend.governance.guard_system.protected_paths`` — imports the
...
#: Producer for the ``stability_gate`` Verify-Stage (FK-05 §5.10/§5.14,
#: FK-37 §37.1.3). Registered in the AG3-064 StageDefinition registry via
#: ``verify_system.stage_registry.data.STANDARD_STAGES``. SSOT for the
#: wire string (ARCH-55, no second naming truth).
STABILITY_GATE_PRODUCER: str = "verify-system.stability-gate"
