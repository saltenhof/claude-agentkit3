...
live here (core_types), NOT in the protected ``agentkit.backend.governance`` namespace,
...
Callers:
- ``agentkit.backend.governance.principal_capabilities.paths`` — imports the strings
...
from agentkit.backend.core_types.qa_artifact_names import HANDOVER_FILE
...
#: ``agentkit.backend.governance`` namespace nor the ``state_backend`` duplicates the
