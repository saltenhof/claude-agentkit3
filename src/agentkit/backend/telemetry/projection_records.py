...
the respective top-surface (``agentkit.backend.closure`` resp.
``agentkit.backend.pipeline_engine.phase_executor``, AC001-compliant) so telemetry-and-events
...
def __getattr__(name: str) -> Any:
    ...
    ``agentkit.backend.closure`` has long been fully loaded.
