...
- :class:`HookContext` -- the harness-neutral observation a hook reacts to. It
  ...
  :class:`agentkit.backend.governance.guard_evaluation.HookEvent` (the guard-side event)
  so that the telemetry hooks stay within the AC10 import boundary (core_types,
  telemetry, and the verdict types from ``agentkit.backend.governance.protocols`` — the
...
- :class:`HookResult` -- the immutable outcome of :meth:`TelemetryHook.evaluate`.
  ...
  :class:`~agentkit.backend.telemetry.hooks.review_guard.ReviewGuard`, an optional
  :class:`~agentkit.backend.governance.protocols.GuardVerdict`.
...
governance owner :class:`agentkit.backend.governance.guard_system.WebCallBudgetGuard`
(FK-30 §30.5.1a); the :class:`~agentkit.backend.telemetry.hooks.budget_event_emitter.BudgetEventEmitter`
...
class EmittingHook:
    ...
    persistence path is the canonical :class:`~agentkit.backend.telemetry.emitters.EventEmitter`
