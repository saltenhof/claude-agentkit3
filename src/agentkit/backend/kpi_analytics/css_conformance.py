...
# Pure data definitions (regexes, status whitelists, rule ids, token prefixes)
# live in the sibling patterns module so this module stays focused on the parser
# and state machine (Sonar PY_MODULE_TOP_LEVEL_MAX_LOC).  Names are unchanged.
from agentkit.backend.kpi_analytics._css_conformance_patterns import (
    _ALL_STATUS_VARS,
    _ALLOWED_STATUS_MAP,
    _CLOSE_BRACE,
    _CONTROL_PROP_PREFIX,
    _CONTROL_SELECTOR_RE,
    _CONTROL_SIZE_PROP_RE,
    _CSS_VAR_DECL_RE,
    _FONT_SIZE_VALUE_RE,
    _HEX_COLOR_RE,
    _LOCAL_SIZE_DECL_RE,
...
def _get_owner_token_names() -> frozenset[str]:
    ...
    from agentkit.backend.kpi_analytics.design_system import build_css_variables, get_design_system
