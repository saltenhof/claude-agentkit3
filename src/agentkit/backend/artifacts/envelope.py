...
- ``ENVELOPE_SCHEMA_VERSION = "3.0"`` — wire-schema version of this
  ...
  ``agentkit.backend.artifacts`` BC and is immutable to external
  storage-schema migrations.

- ``agentkit.backend.state_backend.config.SCHEMA_VERSION`` (e.g. ``"3.3.0"``) —
...
from agentkit.backend.artifacts.producer import Producer
...
#: Not to be confused with ``agentkit.backend.state_backend.config.SCHEMA_VERSION``
...
#: (``agentkit.backend.exploration.change_frame``), so a frame ``story_id`` and the
