...
* :data:`~agentkit.backend.telemetry.events.EventType.LLM_CALL` with
  ``role=adversarial_sparring`` (the pool-send fact, FK-11 §11.8.2), and
* :data:`~agentkit.backend.telemetry.events.EventType.ADVERSARIAL_SPARRING` with a
  ``pool`` field (the adversarial domain event, FK-48 §48.1.6).

This module consumes the AG3-065 verify-LLM-transport (the narrow
:class:`~agentkit.backend.verify_system.llm_evaluator.llm_client.LlmClient` port — its
...
from agentkit.backend.verify_system.adversarial_orchestrator.runtime.models import SparringProof
