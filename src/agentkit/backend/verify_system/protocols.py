...
Since AG3-021, Severity comes from ``agentkit.backend.core_types`` and uses the
...
class StoryContextQueryPort(Protocol):
    ...
    ``agentkit.backend.state_backend.store`` import inside ``verify_system``. The
...
class Finding:
    ...
    Args:
        ...
        finding_type: Optional FK-48 §48.2.2 finding type (additive). When set
            ...
            (:func:`~agentkit.backend.verify_system.adversarial_orchestrator.spawn.extract_mandatory_targets`).
