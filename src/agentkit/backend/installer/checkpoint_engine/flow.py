...
Spine + branches (FK-50 §50.3.1, story §2.1.1):
    ...
    cp_01 -> cp_02 -> cp_03 -> cp_04 -> cp_05 -> cp_06 -> cp_07 -> cp_08
          -> cp_09 -> cp_10 -> cp_10a -> branch_are_enabled
...
- ``cp_10_mcp_registration`` precedes ``cp_10a`` and ``cp_10c`` (it is the
  common MCP precondition — mandatory story-knowledge-base MCP plus ARE-MCP at
  ``features.are``, FK-03 §3.1).
- ``cp_10b_concept_validation_hook`` follows ``cp_11_git_hooks_and_claude``;
  CP 11 owns only CLAUDE.md, while CP 10b atomically writes, verifies and then
  activates the git-hook pair.

VectorDB has no optional flow branch: configuration validation rejects the only
former opt-out before this graph is constructed.
"""
