...
class StateBackendProjectApiTokenRepository(ProjectApiTokenRepository):
    """Persist project API tokens through the canonical state-backend facade."""

    def __init__(self, store_dir: Path | None = None) -> None:
        self._store_dir = store_dir or Path.cwd()

    def get(self, token_id: str) -> ProjectApiToken | None:
        return facade.load_project_api_token(token_id, self._store_dir)

    def get_by_hash(self, token_hash: str) -> ProjectApiToken | None:
        return facade.load_project_api_token_by_hash(token_hash, self._store_dir)

    def list_for_project(self, project_key: str) -> list[ProjectApiToken]:
        return facade.load_project_api_tokens_for_project(project_key, self._store_dir)

    def save(self, token: ProjectApiToken) -> None:
        facade.save_project_api_token(token, self._store_dir)
