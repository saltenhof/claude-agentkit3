...
Implements ``agentkit.backend.governance.repository.WorktreeRepository`` by loading
the ``StoryContext`` from the state backend and extracting the
``worktree_map`` field.  This keeps
``agentkit.backend.governance.runner.Governance`` decoupled from the state backend
...
class StateBackendWorktreeRepository:
    ...
    Args:
        project_root: Root directory used to resolve the story directory via
            ``agentkit.backend.installer.paths.story_dir``.  When ``None``, falls
            back to ``Path.cwd()``.
    """
