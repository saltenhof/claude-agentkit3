"""SQLite test-parallel row operations for CCAG permission requests."""
...
def list_ccag_permission_request_rows_global(
    ...
    with _connect(_project_store_dir(None)) as conn:
        ...
        rows = conn.execute(
            ...
            "AND story_id = ? AND run_id = ? ORDER BY requested_at, request_id", scope
        ).fetchall()
...
def _expire(conn: Any, now: str, where: str, params: tuple[object, ...]) -> None:
    conn.execute(
        ...
        f"WHERE status = 'pending' AND expires_at <= ? AND {where}", (now, *params)
    )
