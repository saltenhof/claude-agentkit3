...
from agentkit.backend.governance.governance_observer.config import (
    get_cooldown_s,
    get_risk_threshold,
    get_window_size,
)
...
if TYPE_CHECKING:
    from agentkit.backend.config.models import GovernanceConfig
    from agentkit.backend.failure_corpus.top import FailureCorpus
    from agentkit.backend.governance.governance_observer.adjudicator import GovernanceAdjudicatorPort
...
class GovernanceObserver:
    ...
    Args:
        ...
        failure_corpus: Optional :class:`~agentkit.backend.failure_corpus.top.FailureCorpus`
            for incident handoff (FK-35 §35.3.9).
        config: Optional :class:`~agentkit.backend.config.models.GovernanceConfig`
...
def _validate_window_payloads(payloads: list[dict[str, object]]) -> None:
    ...
    Applies :func:`~agentkit.backend.governance.governance_observer.score._validate_signal_payload`
    ...
    Raises:
        ...
        agentkit.backend.governance.governance_observer.score.GovernanceSignalPayloadError:
