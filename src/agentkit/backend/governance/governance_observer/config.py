...
``governance.cooldown_s`` are stored in :class:`~agentkit.backend.config.models.GovernanceConfig`
...
def get_window_size(cfg: GovernanceConfig | None) -> int:
    ...
    Args:
        cfg: Optional :class:`~agentkit.backend.config.models.GovernanceConfig`.
