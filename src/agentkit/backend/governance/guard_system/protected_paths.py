...
Canonical location: ``agentkit.backend.governance.guard_system.protected_paths``.
...
- BC-Cut §BC 4 positions the GuardSystem under ``agentkit.backend.governance.guard_system``.
...
Truth-boundary discipline: ``agentkit.backend.governance`` is a
...
constants in ``agentkit.backend.core_types.qa_artifact_names`` and are only imported
...
from agentkit.backend.state_backend.paths import STATE_DB_DIR, STATE_DB_FILE
