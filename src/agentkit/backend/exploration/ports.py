...
``agentkit.backend.exploration`` is a bloodgroup-A domain core
...
``agentkit.backend.state_backend.store`` directly. The persisted change-frame and the
...
class WorkerDraftPresenceReader(Protocol):
    ...
    the draft (that is :class:`~agentkit.backend.exploration.drafting.drafting.ExplorationDrafting`'s
...
class DeclaredImpactReader(Protocol):
    ...
    :class:`~agentkit.backend.story_context_manager.story_model.Story` model (the GitHub
