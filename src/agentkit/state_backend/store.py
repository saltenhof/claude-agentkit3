...
def load_story_context(story_dir: Path) -> StoryContext | None:
    with _connect(story_dir) as conn:
        row = conn.execute(
            "SELECT payload_json FROM story_contexts LIMIT 1",
        ).fetchone()
    if row is None:
        return None
    try:
...
def load_phase_state(story_dir: Path) -> PhaseState | None:
    with _connect(story_dir) as conn:
        row = conn.execute(
            "SELECT payload_json FROM phase_states LIMIT 1",
        ).fetchone()
    if row is None:
        return None
    try:
...
def load_flow_execution(story_dir: Path) -> FlowExecution | None:
    with _connect(story_dir) as conn:
        row = conn.execute("SELECT * FROM flow_executions LIMIT 1").fetchone()
    if row is None:
        return None
