...
(``agentkit.exploration``) must not import ``state_backend.store`` nor touch the
...
* ``write_change_frame_file`` materializes the change-frame as
  ...
  via :func:`agentkit.utils.io.atomic_write_text`) -- the file the
...
class StateBackendExplorationChangeFrameAdapter:
    ...
    def write_change_frame_file(
        ...
        :func:`agentkit.utils.io.atomic_write_text`); a crash never leaves a
