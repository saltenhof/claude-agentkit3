...
def prompt_template_path(name: str) -> Path:
    ...
    relpath = _manifest_entry(name)["relpath"]
...
def prompt_template_relpath(name: str) -> str:
    """Return the bundle-relative path of a prompt template."""

    return _manifest_entry(name)["relpath"]
...
def prompt_template_sha256(name: str) -> str:
    ...
    content = load_prompt_template(name).encode("utf-8")
    digest = hashlib.sha256(content).hexdigest()
    expected = _manifest_entry(name)["sha256"]
    if digest != expected:
        raise ProjectError(
