...
if TYPE_CHECKING:
    ...
    from agentkit.story_context_manager.story_model import Story, StorySpecification
...
class StoryIndexPort(Protocol):
    ...
    :class:`~agentkit.integrations.vectordb.VectorDbError` on failure (hard
    blocker, fail-closed).
    """
    ...
    *,
    ...
    try:
        ...
        return StoryMdExportResult(
            ...
            file_size_bytes=size,
            error=(
                f"Weaviate indexing failed: {exc} (fail-closed: indexing failure "
                "blocks the export, no catch-up path, FK-21 §21.11.4)"
            ),
        )
