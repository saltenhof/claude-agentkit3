...
3. Validates the response against the dedicated
   :class:`~agentkit.governance.governance_observer.models.GovernanceAdjudicationVerdict`
   schema (fail-closed on schema violation).

It does NOT reuse :class:`~agentkit.verify_system.llm_evaluator.structured_evaluator.StructuredEvaluator`
...
if TYPE_CHECKING:
    ...
    from agentkit.multi_llm_hub.entities import HubBackendName, HubMessage
...
class HubGovernanceAdjudicator:
    ...
    Args:
        hub_client: The :class:`~agentkit.multi_llm_hub.client.HubClientProtocol`
