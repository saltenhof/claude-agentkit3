...
of the :class:`~agentkit.governance.governance_observer.score.ExecutionEventReader`
...
* **Public run-scoped facade reader** — this reader calls the public
  ``agentkit.state_backend.store.facade.load_execution_events`` (signal reads) and
  ``agentkit.state_backend.store.facade.load_last_adjudication_ts`` (cooldown reads).
    ...
    * :meth:`read_last_adjudication_ts` — returns the UNIX timestamp of the last
    ...
    def read_last_adjudication_ts(
        ...
        *,
        ...
        field matching via :func:`~agentkit.state_backend.store.facade.load_last_adjudication_ts`
        ...
        """
        from agentkit.state_backend.store.facade import load_last_adjudication_ts
    ...
    def _load_signal_records(
        ...
        *,
        ...
        Returns:
            List of :class:`~agentkit.telemetry.contract.records.ExecutionEventRecord`.
        """
        from agentkit.state_backend.store.facade import load_execution_events
...
def _assert_story_dir_valid_for_backend(story_dir: Path | None) -> None:
    ...
    Uses :func:`~agentkit.state_backend.store.facade.active_backend_is_sqlite`
    (the sanctioned facade surface) rather than importing
    ``agentkit.state_backend.config`` directly — preserving AC010/AC011 (GAC-1).
    ...
    if story_dir is not None:
        return
    from agentkit.state_backend.store.facade import active_backend_is_sqlite
