...
#: Trailing path components that identify THIS package's module file inside a
#: distribution record or below an editable source root.
_PACKAGE_INIT_PARTS: tuple[str, str] = ("agentkit", "__init__.py")
...
def _recorded_package_path(candidate: Distribution) -> Path | None:
    ...
    for entry in candidate.files or ():
        ...
        if parts[-2:] == _PACKAGE_INIT_PARTS:
...
def _editable_source_root(candidate: Distribution) -> Path | None:
    ...
    try:
        ...
        # ``json.JSONDecodeError`` is a ``ValueError`` subclass and is therefore
        # already covered; naming it again would only pretend to add coverage.
        return None
...
def _distribution_owns_current_package(candidate: Distribution) -> bool:
    ...
    return any(
        ...
        for expected in (
            source_root.joinpath("src", *_PACKAGE_INIT_PARTS),
            source_root.joinpath(*_PACKAGE_INIT_PARTS),
        )
    )
