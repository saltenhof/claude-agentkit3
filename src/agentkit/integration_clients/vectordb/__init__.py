...
``agentkit.backend.vectordb``). ``weaviate-client`` is a MANDATORY base dependency
(FK-13 §13.1); the adapter still fails closed with a typed error when it is
absent or Weaviate is unreachable, never a silent empty result.
"""
