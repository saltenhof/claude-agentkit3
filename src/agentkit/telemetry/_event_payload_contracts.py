...
Raw data sibling of ``agentkit.telemetry.events`` — extracted to keep
``events.py`` well under the PY_MODULE_TOP_LEVEL_MAX_LOC_100 threshold.
Re-exported via ``agentkit.telemetry.events`` so all existing imports remain
unchanged (``from agentkit.telemetry.events import MANDATORY_PAYLOAD_FIELDS``).

No imports from ``agentkit.telemetry.events`` here — that would cause a
