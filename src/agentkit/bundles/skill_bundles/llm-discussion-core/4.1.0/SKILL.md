...
```
Lies `session_id` und `result` direkt aus der JSON-Antwort des CLI-Aufrufs.
...
```bash
claude -p "<Sub-Agent-Init-Prompt>" --output-format json --model opus
```
`session_id` und `result` aus der JSON-Antwort lesen. Die `session_id` wird fuer
ALLE weiteren Interaktionen benoetigt.
...
```bash
claude -p "RUNDE 1 — Unabhaengige Positionsbildung.
...
```
Die Claude-Position aus `result` der JSON-Antwort lesen.
...
An Claude-Sub-Agent via CLI `--resume`:
```bash
claude -p "RUNDE {N}. [DEINE ROLLE: {aktuelle_rolle}]
...
```bash
claude -p "<Rollentausch-Prompt>" --resume "<CLAUDE_SESSION_ID>" --output-format json --model opus
```
Die Claude-Bestaetigung aus `result` der JSON-Antwort lesen.
...
```bash
claude -p "<Init-Prompt unten>" --output-format json --model opus
```

**`session_id` und `result` aus der JSON-Antwort lesen. Die `session_id` wird fuer
ALLE weiteren Interaktionen via `--resume` benoetigt.**
