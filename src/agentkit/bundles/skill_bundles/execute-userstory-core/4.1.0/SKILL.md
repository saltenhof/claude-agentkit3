...
```bash
{{AK3_WRAPPER}} run-phase exploration \
  --story <STORY-ID> \
...
3. Call `{{AK3_WRAPPER}} run-phase exploration --story <STORY-ID>` again
...
The Worker will write `bugfix-reproducer.json`; the verify phase validates the
reproducer contract and persists the resulting evidence.
...
IF "status" == "NEEDS_CLARIFICATION":
  ...
  3. Write user answers to _temp/qa/<STORY-ID>/clarification-answers.json as JSON:
     {
       "schema_version": "1.0",
       "story_id": "<STORY-ID>",
       "answers": [
...
- `status` + `verify_result` together determine the next action:
  - `status = "PAUSED"` + `verify_result = "RUN_SEMANTIC"`: spawn QA agents from `agents_to_spawn`, then call `{{AK3_WRAPPER}} run-phase verify` again (see step [5])
...
**Do NOT invoke a policy module separately** — `run-phase verify` handles the
policy decision in the resume path.
...
```
{{AK3_WRAPPER}} run-phase {setup|exploration|implementation|verify|closure}
tools/orchestration/compose-prompt.py
agentkit.backend.governance.github_status            (was: tools/governance/set-github-status.sh)
agentkit.core.context                        (was: tools/governance/compute-story-context.sh)
agentkit.backend.governance.artifacts                (was: tools/governance/verify-worker-artifacts.sh)
agentkit.backend.closure.closure                     (was: tools/closure/story-closure.sh)
...
| Phase | Tool | Type |
|-------|------|------|
| Phase 1 | `{{AK3_WRAPPER}} run-phase setup` | Deterministic (phase orchestration) |
| Phase 1 | `agentkit.core.context` | Deterministic (context extraction) |
| Phase 1 | `agentkit.backend.governance.github_status` | Deterministic (status update) |
...
| Phase 3 | `{{AK3_WRAPPER}} run-phase verify` | Deterministic (phase orchestration) |
| Phase 3 | `agentkit.backend.governance.artifacts` | Deterministic (artifact check) |
...
| Phase 5 | `{{AK3_WRAPPER}} run-phase closure` | Deterministic (phase orchestration) |
...
| Phase 5 | `agentkit.backend.closure.closure` | Deterministic (metrics+close) |
