...
allowed-tools: "Bash, Read, AskUserQuestion, Write, mcp"
---
...
## Step 0: Read Project Rules + Mandatory VectorDB Boundary

Read `CLAUDE.md` from the project root first. Then load
`.agentkit/config/project.yaml` and retain its strictly configured
`project_key`, `project_prefix`, `concepts_dir`, `wiki_stories_dir`, and both
`pipeline.vectordb` endpoints. Do not infer literal directory names and do not
use environment, localhost, or installer defaults in place of this project
configuration.

Run the project-bound preflight:

```bash
{{AK3_INTERPRETER}} -m agentkit.backend.vectordb.wait_for_weaviate \
  --project-root "$PROJECT_ROOT" --timeout 10
```

A missing or malformed config, missing endpoint, unreachable endpoint,
non-Weaviate service, readiness failure, or incompatible version is a hard
stop. Do not start a container and do not continue with local discovery.
Indexing and duplicate reconciliation remain owned by the backend/engine ports;
the skill never calls Weaviate directly.

## Step 1: Determine Story Type

Evaluate from conversation context which of the **four story types** this is:

- **Implementation**: New functionality, enhancement, infrastructure — produces code + tests. Refactoring may occur as a technique inside implementation work, but is not its own story type. ID schema: `$CONFIGURED_PROJECT_PREFIX-{NNN}`.
- **Concept**: Analysis, design work, architectural proposals — produces documents, NO code. ID schema: `$CONFIGURED_PROJECT_PREFIX-{NNN}`.
- **Bugfix**: Fixing a defect in existing code — produces code + reproducing test. ID schema: `$CONFIGURED_PROJECT_PREFIX-FIX-{NNN}`.
- **Research**: Investigation, experiment, evaluation — produces findings documents, code optional (prototypes). ID schema: `$CONFIGURED_PROJECT_PREFIX-{NNN}`.
...
2. Call the `story_search` MCP tool (semantic VectorDB search over the AK3
   ...
   ```
3. A missing MCP registration, tool error, unavailable VectorDB, missing
   freshness evidence, or stale index is a hard stop. Do not enumerate or text
   search `wiki_stories_dir` as a substitute.
...
- Konzeptquellen aus der verpflichtenden `concept_search`-Discovery unter dem konfigurierten `concepts_dir` (ganze Dokumente, keine Kapitelreferenzen; Ausschlüsse mit Begründung dokumentieren.)
...
- Dependencies (other story IDs, or "Keine")
- Konzeptquellen aus der verpflichtenden `concept_search`-Discovery; wenn keine Quelle anwendbar ist, alle Kandidaten mit Ausschlussbegründung dokumentieren.
...
- Konzeptquellen aus der verpflichtenden `concept_search`-Discovery einschließlich anwendbarer Grundlagen-, Companion- und Appendix-Dokumente.
...
```
Read $CONFIGURED_WIKI_STORIES_DIR/story-labels.md
```
...
(Requirements) fuer Konzept-Referenzen. Konzeptquellen liegen ausschließlich
unter dem in `project.yaml` konfigurierten `concepts_dir`.

### 3b.1 — Semantische Suche an der registrierten MCP-Grenze

`concept_search` ist der einzige Discovery-Pfad. Rufe das Tool fuer Titel und
Scope auf:
...
Rufe es außerdem für jedes Akzeptanzkriterium auf:

```
concept_search(query="<AC-Text>", limit=5)
```

Verwende die Ergebnisse für Hauptkonzepte, Companion-/Appendix-Beziehungen und
Authority-Deferrals. Ein Toolfehler, fehlende Registrierung, fehlende
Completion oder eine nicht aktuelle Revision ist ein harter Stopp. Es gibt
keinen lokalen oder manuellen Discovery-Ersatzpfad.

### 3b.2 — Trigger-Rules anwenden
...
### 3b.3 — Authority-Deferrals verfolgen
...
### 3b.4 — Exclusion Rationale (Pflicht)
...
|---------|-----------|-----|-------------|
| {configured_concepts_dir}/TK-07-error-routing.md | Kap. 2+3 | primary | Error-Routing-Spez. |
| {configured_concepts_dir}/TK-07-appendix.md | Appendix I.4 | appendix | Referenz-Impl. |
| {configured_concepts_dir}/TK-01-base-state.md | Kap. 1 | foundational | State-Schema-Modif. |

Excluded (mit Rationale):
| Konzept | Abschnitt | Begruendung |
|---------|-----------|-------------|
| {configured_concepts_dir}/TK-09-ui.md | Kap. 4 | Keine UI-Aenderung in dieser Story |
...
1. Lese `.agentkit/config/project.yaml` → `repositories`-Sektion.
...
3. Ordne jeden Dateipfad einem Repo zu:
   ...
   - Dateien im Projekt-Root (z.B. der konfigurierte Concept-/Story-Pfad oder `docs/`) gehoeren zum Root/Wiki-Repo
4. Bestimme:
...
6. Wenn ein Dateipfad keinem Repo zugeordnet werden kann: Warnung an den User — das Root/Wiki-Repo
   fehlt moeglicherweise in `.agentkit/config/project.yaml`.
...
- Implementation/Concept/Research: `$CONFIGURED_PROJECT_PREFIX-{NNN}`
- Bugfix: `$CONFIGURED_PROJECT_PREFIX-FIX-{NNN}`
...
## Concept References
- {configured_concepts_dir}/XX-yyy.md - Chapter N - primary - primary specification
- {configured_concepts_dir}/ZZ-zzz.md - Chapter N - excluded on purpose; document here only if that exclusion matters for scope

## Guardrail References
- Project guardrails (guardrails/architecture-guardrails.md and guardrails/testing-guardrails.md) apply.
...
- Concept fidelity: no implicit deviation from the configured concept corpus; conflicts surfaced explicitly.
```
...
## Concept References
- {configured_concepts_dir}/fachkonzept.md - Chapter N - primary - input for this analysis
...
- Output is concept-faithful and consistent with the configured concept corpus; conflicts surfaced explicitly.
...
1. Beginnt der Pfad mit dem konfigurierten `$CONFIGURED_CONCEPTS_DIR/`?
...
PFAD="{PFAD}"
if [[ "$PFAD" != "$CONFIGURED_CONCEPTS_DIR/"* ]]; then
  echo "FEHLER: $PFAD liegt nicht im konfigurierten Concept-Verzeichnis"
  exit 1
fi
if [[ "$PFAD" == *..* ]]; then
...
Validiere zuerst den aktuellen, project-configured Concept-Korpus:

```bash
{{AK3_WRAPPER}} concept \
  --concepts-dir "$PROJECT_ROOT/$CONFIGURED_CONCEPTS_DIR" \
  validate --corpus --strict
```

Der JSON-Output muss Exit-Code 0, `status: "valid"` und einen nichtleeren
String `corpus_revision` liefern. Merke diesen Wert als
`VALIDATED_CONCEPT_REVISION`.

Rufe anschließend das registrierte MCP-Tool auf:

```
story_list_sources(project_id="<project_prefix aus project.yaml>")
```

Der Response muss genau einen Source-Eintrag mit `source_type: "concept"`,
einem nichtleeren String `last_revision` und ganzzahligem
`stale_chunk_count: 0` enthalten. Vergleiche
`last_revision == VALIDATED_CONCEPT_REVISION`. Damit wird die aktuelle,
validierte Graph-/Corpus-Revision gegen die retrieval-maßgebliche
Concept-Completion derselben AG3-174-Engine geprüft.

Fehlende oder mehrfach vorhandene Concept-Einträge, fehlende/falsch typisierte
Felder, eine leere Completion, `stale_chunk_count != 0`, Revisions-Mismatch,
CLI-/MCP-Fehler oder nicht erfolgreicher Exit-Code sind jeweils ein harter
Stopp. Rufe erst danach `concept_search` für den finalen Story-Body erneut auf.
Es gibt keinen manuellen Body-to-Reference- oder lokalen Discovery-Ersatz.
...
3. **Auswerten:**
   ...
   - Bei `NEEDS_CONTEXT: <Liste>`:
     a. `concept_search(query="<Begriff>", limit=5)` für jeden genannten Begriff
        aufrufen. Ein Tool- oder Freshness-Fehler ist ein harter Stopp.
...
```bash
{{AK3_INTERPRETER}} tools/agentkit/projectedge.py create-story \
  --project-key "$CONFIGURED_PROJECT_KEY" \
  --title "<Title>" \
...
STORY_DIR="$CONFIGURED_WIKI_STORIES_DIR/${STORY_ID}_<kebab-case-slug>"
mkdir -p "$STORY_DIR"
```
...
VectorDB indexing failure), report the error. Writing `story.md` manually or
skipping indexing is forbidden.
...
- Wiki: $CONFIGURED_WIKI_STORIES_DIR/<STORY_DIR>/story.md
