...
```bash
python -m agentkit.backend.vectordb.wait_for_weaviate --timeout 10 --project-root .
```

If exit code is NOT 0: **HARD STOP.** Inform the user with the named cause from
stderr (missing/malformed endpoint, not ready, not Weaviate, version, …).
The installer does **not** start a database — the operator must provide a
running Weaviate at the configured endpoint. **No localhost default** on the
project-bound path.

Do NOT proceed with story creation if this check fails. Do NOT invent endpoints.

**VectorDB Facts (DO NOT hallucinate these):**
- Weaviate collection name: **`StoryContext`** (NOT "StoryChunk", NOT "Stories")
- Endpoint: from `.agentkit/config/project.yaml` → `pipeline.vectordb.host/port/grpc_port` (required)
...
VectorDB — never against GitHub issues. VectorDB is **mandatory infrastructure**
(FK-13 §13.1); there is **no** Grep/filesystem fallback path.

### Semantic Search (VectorDB MCP — only productive path)
...
2. Call the `story_search` MCP tool (semantic VectorDB search over the AK3
   ...
   ```
3. **HARD STOP** if `story_search` is unavailable or returns a VectorDB/tool error.
   Do **not** fall back to Grep, `ls`, or directory scans.
...
**Konfiguriertes `concepts_dir`:** Lies `.agentkit/config/project.yaml` strikt (fail-closed; derselbe Loader wie der Installer). Nimm den Wert des Feldes `concepts_dir` (Projekt-relativ). Verwende **dieses** Verzeichnis — niemals stillschweigend einen Default-Pfad erfinden. **CONCEPTS_DIR** = dieser gelesene Wert (kein Materialisierungs-Platzhalter).


### 3b.0 — Freshness- / Graph-Hard-Stops (VOR jeder Suche)

VectorDB + Concept-Graph sind **Pflichtinfrastruktur** (FK-13 §13.1 / §13.8).
**Kein Grep-/Dateiscan-Fallback** (AG3-176 AC7). Bei jedem der folgenden
Zustaende: **HARD STOP** — Story-Erstellung abbrechen und dem User die
Ursache melden:

| Bedingung | Aktion |
|-----------|--------|
| `concept_search` MCP-Tool nicht im Skill-Kontext aufrufbar | HARD STOP |
| `CONCEPTS_DIR/concept_graph.json` fehlt | HARD STOP (`graph_unavailable`) |
| Graph `corpus_revision` weicht vom Live-Corpus ab (stale) | HARD STOP (`graph_stale`) |
| `corpus_revision`-Mismatch in Tool-Antwort vs. Graph | HARD STOP |
| VektorDB-/Toolfehler (`vectordb_*`, transport, parse) | HARD STOP |
| Konfiguriertes `concepts_dir` weicht vom Runtime-Binding ab | HARD STOP |

Do **NOT** fall back to Grep, Glob, or manual directory scans to paper over
these failures.

### 3b.1 — INDEX.yaml lesen (nach erfolgreichem Graph-Gate)

```
Read CONCEPTS_DIR/INDEX.yaml
```

Wenn INDEX.yaml fehlt **nach** erfolgreichem Graph-Build im Zielprojekt:
HARD STOP — der Post-Commit-Owner muss `concept build` geliefert haben
(FK-13 §13.9.9). Kein stiller Glob-Fallback als Ersatz fuer den Graph.

### 3b.2 — Semantische Concept-Suche (Primaerpfad, Pflicht)

**Einziger regulaerer Suchpfad:** MCP-Tool `concept_search` gegen den
indizierten Concept-Korpus:
...
`concept_search` ist ein VectorDB-MCP-Tool des `story-knowledge-base`-Servers.
Es **muss** verfuegbar und frisch sein. Bei Ausfall: HARD STOP (siehe 3b.0) —
**kein** Grep-Rueckfall.

### 3b.3 — Exakt-Nachschlagen ueber concept_search (nicht Grep)

Klassen-/Interface-Namen, Konfig-Keys, Event-Typen aus den Technischen Details
ebenfalls ueber `concept_search` (Query = Identifier). Kein Grep-Fallback.
...
3. **Auswerten:**
   ...
   - Bei `NEEDS_CONTEXT: <Liste>`:
     a. Direkte Datei-Reads der genannten Dokumente (Pflichtpfad).
        Pflicht: `concept_search(query="<Begriff>", limit=5)` fuer jeden genannten
        Begriff. Fehlt das Tool oder schlaegt es fehl: HARD STOP (kein Grep-Fallback).
