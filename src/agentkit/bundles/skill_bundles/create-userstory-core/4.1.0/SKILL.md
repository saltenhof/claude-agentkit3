...
```bash
python -m agentkit.backend.vectordb.wait_for_weaviate \
...
PFAD="{PFAD}"
case "$PFAD" in
  "$CONFIGURED_CONCEPTS_DIR"/*) ;;
  *) echo "FEHLER: $PFAD liegt nicht im konfigurierten Concept-Verzeichnis"; exit 1 ;;
esac
if [[ "$PFAD" == *..* ]]; then
...
```bash
python -m agentkit.backend.vectordb.cli \
