...
| closure | `ClosurePayload` | `progress: ClosureProgress` | Fortschritt der Closure-Substates: `integrity_passed`, `merge_done`, `story_closed`, `metrics_written`, `postflight_done` (je `bool`). [Hinweis: Fuer Concept/Research-Stories ohne Worktree/Branch werden `integrity_passed` und `merge_done` vom Phase Handler direkt auf `true` gesetzt (kein Branch vorhanden = nichts zu pruefen/mergen). Detaillierte Closure-Logik in FK-29 §29.1.] |
