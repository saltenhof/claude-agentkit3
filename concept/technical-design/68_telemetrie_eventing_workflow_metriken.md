...
glossary:
  exported_terms:
    ...
    - id: review-guard
      definition: >
        Observational-Hook (telemetry/review_guard.py), der nach jeder
        Review-/LLM-Anfrage prueft, ob der Review ueber ein genehmigtes Template-Sentinel
...
| Event | Wann | Zusatzfelder | Erwartungswert | Quelle |
|-------|------|-------------|----------------|--------|
| `review_request` | Worker fordert Review von Pflicht-LLM an | `pool`, `role` | Mindestens 1 pro Story | Review-Flow mit Review-Template |
| `review_response` | Pflicht-LLM liefert Review-Ergebnis | `pool`, `role` | Gleiche Anzahl wie review_request | Review-Flow |
...
| `adversarial_sparring` | Adversarial nutzt freiwilliges Harness-Sparring | `pool` | Optional, kein Gate-Nachweis | Hook (PostToolUse für Harness-Send) |
...
| Event | Wann | Zusatzfelder | Erwartungswert | Quelle |
|-------|------|-------------|----------------|--------|
| `preflight_request` | Preflight-Prompt an den LLM-Hub gesendet. | `pool` | >= 1 pro Story (Pflicht) | AK3 Review-Flow über FK-75 |
| `preflight_response` | Preflight-Antwort vom LLM empfangen. | `pool`, `request_count` | == preflight_request count | AK3 Review-Flow über FK-75 |
...
| LlmCallHook | `agentkit.backend.telemetry.hooks.llm_call` | LLM-Request | Story aus aktivem Run im State-Backend | `llm_call` |
...
| ReviewSentinelHook | `agentkit.backend.telemetry.hooks.review_sentinel` | Review-/LLM-Request | Review-Template-Sentinel erkannt | `review_request`, `review_response` |
| ReviewGuard | `agentkit.backend.telemetry.hooks.review_guard` | Review-/LLM-Request | Template-Sentinel-Pattern | `review_compliant` |
...
einem Sentinel-Pattern in der Review-/LLM-Anfrage:
