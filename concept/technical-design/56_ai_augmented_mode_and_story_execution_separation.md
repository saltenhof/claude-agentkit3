  ...
  exported_terms:
    ...
    - id: ownership-transfer
      definition: >
        ...
        unter gleichem run_id und denselben worktree_roots
        fortgefuehrt; die entmuendigte Session geht nach
        binding_invalid mit Grund ownership_transferred.
      see_also:
        - term: run-binding
          domain: story-lifecycle
...
- A's Bindung revoked; neue Bindung fuer B's `session_id` auf
  denselben `run_id` und dieselben `worktree_roots`, mit neuer
  `binding_version`;
- Tombstone fuer A's lokales Edge-Bundle (Invalidierung der lokalen
  Projektion, analog §56.12);
- Takeover-Snapshot als Audit-Fakt festgehalten; das normierte
  Detail-Schema liegt beim API- und Speichervertrag (FK-91 bzw.
...
frueherer Mutationen — bleiben A erlaubt.
...
einem Run zugeordnet; Worktree und Branch gehoeren fachlich der
Story, nicht der Session. B's neue Bindung uebernimmt deshalb
dieselben `worktree_roots` — die Invariante
`story_execution_requires_lock_binding_and_worktree_match` erzwingt
damit, dass B im vorhandenen Worktree arbeitet. Der Arbeitsstand wird
as-is uebergeben; Verwerfen ist eine getrennte, explizite Wahl
(Story-Reset, FK-53), kein Takeover-Bestandteil. Die Run-Fortfuehrung
ist nur zulaessig, weil die neue `ownership_epoch` als Fence in allen
