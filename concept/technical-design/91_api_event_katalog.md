...
| `/v1/governance/capability-adjudications` | `POST` | Eine Prinzipal-Operation adjudizieren, bevor der Hook ein Werkzeug freigibt (FK-55 §55.10.3 Schritte 1-5 als **EIN** Vorgang, nicht fuenf Repository-Spiegel; AG3-239). Der Kern besitzt Capability-Matrix, Prinzipal-Attestierung und den kanonischen Conflict-Freeze-Record (FK-01 §1.1a); der Hook-Prozess besitzt keinen davon. **Request** `agentkit_wire.governance_adjudication.CapabilityAdjudicationRequest`: `op_id` (Regel 5), die harness-neutralen Ereignisfelder (`operation`, `operation_args`, `principal_kind`, `cwd`, `session_id`, `parent_session_id`, `cli_args` — **nie** Prompt-Inhalt), der lokal aufgeloeste Kontext (`execution_mode`, `story_id`, `story_scope_roots`, `binding_revocation_reason`, `new_owner_ref`) und `local_freeze`: was der **lokale** Freeze-Export dieses Rechners sagt. **Response** `CapabilityAdjudicationResponse`: `outcome` (`permit`|`deny`|`unclassified_mutation`|`unknown_permission`|`unresolved`|`fault`), `allowed`, `guard_name`, `message`, `violation_type`, `detail`, `freeze_disagreement`. **Fehlerfaelle:** `400` ungueltiger Payload, `422` fehlendes/leeres `op_id`. Ein Capability-Fault erzeugt **keinen** 5xx, sondern `200` mit `outcome=fault` und `allowed=false` — FK-55 §55.10.5 verlangt eine deterministische Entscheidung, kein mehrdeutiges Transportergebnis. **Fail-closed beim Aufrufer:** Eine Adjudikation, die nicht ERLANGT werden kann (Kern nicht erreichbar, Transportfehler, unlesbare Antwort), blockiert; sie erlaubt nie. **Dual-Materialisierung:** Der Kern vergleicht `local_freeze` gegen den kanonischen Record; jede Abweichung — einseitiger Record, `freeze_version`-Drift, fremde Story, unlesbarer Export — ist ein fail-closed Freeze (Invariante `principal-capabilities.invariant.freeze_has_backend_record_and_local_export`) |
