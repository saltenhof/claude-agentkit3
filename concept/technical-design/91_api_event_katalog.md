...
| `/v1/projects/{project_key}/story-conflict-assessments` | `POST` | Create-time Konflikt-Bewertung einer noch nicht existierenden Story (FK-21 §21.4.1 Schritt 3). Request traegt die Draft-Display-ID, die Story-Beschreibung und die Stufe-1-Kandidaten oberhalb der Aehnlichkeitsschwelle; Response traegt genau ein **binaeres** Verdikt `PASS`/`FAIL`. Die Stufe-1-Suche laeuft dev-lokal, die LLM-Bewertung ausschliesslich im Kern: wer sie lokal ausfuehren koennte, beeinflusst das Gate, das seine eigene Story beurteilt. Verdikt-Ambiguitaet ist Konflikt — ein `PASS_WITH_CONCERNS` der geteilten Layer-2-Aggregation wird kernseitig zu `FAIL` kollabiert und erreicht die Grenze nie. Mutiert keinen kanonischen Zustand und traegt deshalb **kein** `op_id` (Regel 5, gleiche Begruendung wie `/v1/auth/login`). Fehlerfaelle: `400 invalid_story_conflict_assessment_payload`, `404 project_not_registered`, `503 story_conflict_assessment_unavailable` (fail-closed, nie ein Ersatzverdikt). Tenant-Scope und Projekt-Token sind Pflicht; auf dem UI-BFF-Listener nicht exponiert. |
| `/v1/projects/{project_key}/verify-evidence-assemblies` | `POST` | Assemblierung des Review-Bundles aus einem edge-exportierten Evidence-Checkpoint (FK-28 §28.7.1). Request traegt die logischen Repositories mit ihrem Change-Inventar und die content-gebundenen Dateibeobachtungen — **niemals** einen physischen Worktree-Pfad; das Story-Arbeitsverzeichnis loest der Kern aus dem `project_registry` auf (FK-10 §10.2.3). Response traegt `manifest_hash`, `merge_paths` und das kanonische Manifest-Dokument. Der menschliche Recovery-Command §91.1 ist der Adapter darauf, nicht eine zweite Implementierung. Mutiert keinen kanonischen Zustand, kein `op_id`. Fehlerfaelle: `400 invalid_verify_evidence_assembly_payload`, `404 project_not_registered`, `422 verify_evidence_assembly_rejected` (der Checkpoint kann kein Bundle ergeben), `503 verify_evidence_assembly_unavailable`. Tenant-Scope und Projekt-Token sind Pflicht; auf dem UI-BFF-Listener nicht exponiert. |
...
| `/v1/projects/{project_key}/stories/{story_id}` | `GET` | Projekt-skopierte Story-Detailansicht (`StoryDetail`→`StorySummary`, inkl. Wire-Key `story_type`); vom Governance-Hook als **server-vermittelter Story-Typ-Read** konsumiert (FK-24 §24.3.2, FK-10 §10.1.0 I1, AG3-129) — fehlender Record ⇒ `404 story_not_found` ⇒ Hook fail-closed UNRESOLVED. Die Antwort fuehrt zusaetzlich `specification` (`StorySpecification` | `null`, AG3-240): `null` heisst *keine Spezifikation hinterlegt*, nie *nicht geladen*. Dies ist die **einzige** exponierte Story-Detailflaeche — bare `/v1/stories/{story_id}` wird nach der projekt-skopierten Konvention (FK-72 §72.8.1) nicht delegiert, weshalb Need/Solution/Acceptance ohne dieses Feld ausserhalb des Kerns unerreichbar waren. |
...
| `agentkit takeover-request --story {story_id} --run {run_id} --session {session_id} --worktree {path} --reason {reason} --project {project_key} --base-url {url}` | 56 | Duenner REST-Adapter auf `POST …/ownership/takeover-request`; zeigt den vollstaendigen Challenge einschliesslich Verlustkorridor-Pflichttext und mintet `op_id` clientseitig |
...
| `agentkit evidence assemble --project-key {project_key} --project-root {path}` | 26, 28 | Duenner Adapter auf `POST /v1/projects/{project_key}/verify-evidence-assemblies` (§91.1a): liest den edge-exportierten Evidence-Checkpoint, laesst den Kern assemblieren und schreibt das zurueckgegebene Manifest. Kein lokaler Assembler, kein lokal erzeugtes QA-Artefakt |
...
| `run_ownership_takeover_approval_requested` | 56 | Control Plane / Ownership-Takeover-Pfad | Agenteninitiierter Takeover-Request wartet auf menschliche Frontend-Freigabe; Anfrager erhielt `pending_human_approval` (Wire-Schema: `operating-modes.event.run_ownership_takeover_approval_requested`) |
