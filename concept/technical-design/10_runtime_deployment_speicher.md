...
```mermaid
graph TB
    subgraph SESSION["Harness-Session (Claude Code / Codex; FK-76)"]
...
|-----|-------------|---------------|-----------|
| **Harness-Session** (Claude Code / Codex; FK-76) | Minuten bis Stunden | Mensch (CLI `claude` oder `codex`) | Orchestrator, Worker, Adversarial |
...
**Parallelität:** Der Harness (Claude Code / Codex; FK-76) ruft
...
|----------|---------------------|---------|
| Harness-Session (Claude Code / Codex; FK-76) crashed | Worktree existiert, Lock aktiv, Telemetrie unvollständig | PID-basierte Lock-Erkennung (Kap. 02.7). Neuer Run mit neuer `run_id`, bestehender Worktree wird wiederverwendet. |
| Pipeline-Skript crashed | QA-Artefakt möglicherweise unvollständig | Phase Runner kann Phase wiederholen. Idempotente Skripte. |
| Hook-Prozess crashed | Tool-Call wird blockiert (fail-closed: kein exit(0) = blockiert) | Der Harness (Claude Code / Codex; FK-76) behandelt Hook-Fehler als Blockade. Agent erhält Fehlermeldung. |
...
|---------|-------------------|---------|
| LLM-Pools | MCP-Server-Konfiguration in harness-spezifischer Settings-Datei (z. B. `.claude/settings.json` fuer Claude Code, harness-eigenes Aequivalent fuer Codex; FK-76 §76.5) + Pool-Startup-Skripte | 9100/9200/9300/9400 |
