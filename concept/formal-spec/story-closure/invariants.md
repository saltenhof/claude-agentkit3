...
invariants:
  ...
  - id: story-closure.invariant.push_precedes_merge
    scope: process
    rule: the integrated candidate must be pushed to a Jenkins-reachable story/pre-merge ref before any merge into the target branch is attempted; within the Pre-Merge-Scan-und-Merge-Block (FK-29 §29.1a) this ref push always happens inside the merge-serialization lock and never updates main; when the sonarqube_gate is APPLICABLE Jenkins builds/scans exactly that ref and commit before main may be advanced, whereas in the NOT_APPLICABLE cases the pushed ref is validated by the corresponding gate (Dimensions 1-8 without Sonar for a deliberately absent Sonar, or the Sanity-Gate for mode fast)
  ...
  - id: story-closure.invariant.integrity_gate_precedes_merge_block
    scope: process
    rule: when the sonarqube_gate is APPLICABLE, inside the merge-serialization lock the Integrity-Gate (FK-35 §35.2), including Dimension 9 SonarQube-Green attestation verification (FK-35 §35.2.4a), runs AFTER Jenkins has scanned the pushed integrated-candidate ref and produced the commit-bound attestation and must pass BEFORE the ff-only update of main; Dimension 9 only verifies the fresh commit-bound attestation produced by that scan and never runs its own Sonar scan
  ...
  - id: story-closure.invariant.integrated_candidate_scanned_green_before_main_update
    scope: process
    rule: when the sonarqube_gate is APPLICABLE, after integrating the latest main into the story branch and cleaning the workspace, the integrated candidate must be pushed to a Jenkins-reachable story/pre-merge ref, Jenkins must build and scan exactly that ref and commit, the full integrated-candidate Sonar scan must be read green by analysisId (overall-code invariant) with the exception ledger reconciled by single-match, and tree_hash(scan) must equal tree_hash(merge) before main may be updated; a QA-subflow-green branch alone is insufficient because of main-drift
  - id: story-closure.invariant.push_inside_lock_before_ci_scan
    scope: process
    rule: the story/pre-merge ref push always happens inside the merge-serialization lock before the Jenkins build/test/scan boundary; the pushed ref carries exactly the integrated candidate that Jenkins must build and scan, but it does not update main; main may be advanced only after the applicability-resolved gate has passed
