...
invariants:
  ...
  - id: prompt-runtime.invariant.active_run_uses_one_pinned_bundle
    scope: run
    rule: each active run must pin exactly one resolved prompt bundle version and manifest digest before the first prompt invocation and persist that pin as a dedicated run record
