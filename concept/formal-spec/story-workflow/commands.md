...
commands:
  - id: story-workflow.command.run-phase
    ...
    restrictions:
      ...
      - op_id is the idempotency key (a leased owner-scoped claim serializes concurrent same-op_id dispatches; a replay returns the stored result without re-dispatching)
  ...
  - id: story-workflow.command.resume
    ...
    restrictions:
      ...
      - op_id is the idempotency key reserved by the SAME leased owner-scoped claim as run-phase BEFORE the engine resume runs (no double resume); a replay returns the stored result
