...
Der produktive Transport verwendet den bestehenden `HubClient`-Vertrag
mit begrenzten Lease-Epochen. Jede Epoche erwirbt genau einen gemeinsamen
Lease fuer ChatGPT, Gemini, Grok und Qwen, startet frische Conversations,
rotiert nach einem erfolgreichen Parse-/Policy-Checkpoint und fenced
fehlgeschlagene Sends vor einem Retry. Parse- und Transportfehler werden
als benannte ERROR-Befunde behandelt. Befunde tragen stabile Dokument-/
Anker-/Aussage-Referenzen sowie Prompt- und Modellversion. Die
versionierte Baseline unter `concept/_meta/` akzeptiert nur Eintraege mit
konkretem Grund, listet unterdrueckte Befunde weiterhin und meldet
veraltete Eintraege.
