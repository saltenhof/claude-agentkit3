"""Loopback governance client for every guard-dispatch suite (AG3-129, AG3-239).
...
AG3-239 made this shared rather than integration-only. Capability adjudication
moved behind ``/v1``, and a hook process that cannot REACH the core now fails
closed -- correctly, per FK-55 section 55.10.5. Without a core the unit guard
suites therefore stopped exercising guard logic and started measuring the
fail-closed path instead, which they were not written to assert. They get the
same real-component loopback rather than a weaker assertion.
"""
