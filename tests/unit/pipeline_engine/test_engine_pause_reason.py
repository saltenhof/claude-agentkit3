...
from agentkit.pipeline_engine.engine import _coerce_paused_reason
from agentkit.pipeline_engine.phase_envelope.errors import InvalidPauseReasonError


def test_coerce_paused_reason_none_returns_none() -> None:
    """None input -> None output (no pause reason set)."""
    assert _coerce_paused_reason(None, phase_name="setup") is None


def test_coerce_paused_reason_pause_reason_passthrough() -> None:
    """PauseReason input is returned unchanged."""
    pr = PauseReason.AWAITING_DESIGN_REVIEW
    result = _coerce_paused_reason(pr, phase_name="exploration")
    assert result is pr


def test_coerce_paused_reason_known_string_maps_correctly() -> None:
    """Known synonym strings are mapped to the correct PauseReason."""
    result = _coerce_paused_reason("awaiting_design_review", phase_name="exploration")
    assert result is PauseReason.AWAITING_DESIGN_REVIEW


def test_coerce_paused_reason_canonical_upper_case() -> None:
    """Canonical upper-case wire strings are accepted."""
    result = _coerce_paused_reason("GOVERNANCE_INCIDENT", phase_name="setup")
    assert result is PauseReason.GOVERNANCE_INCIDENT


def test_coerce_paused_reason_unknown_string_raises_invalid_pause_reason_error() -> None:
    """Unknown yield_status strings raise InvalidPauseReasonError (fail-closed, AK 5)."""
    with pytest.raises(InvalidPauseReasonError):
        _coerce_paused_reason("completely_unknown_status", phase_name="implementation")
...
def test_coerce_paused_reason_empty_string_raises() -> None:
    """Empty string raises InvalidPauseReasonError (not a silent None)."""
    with pytest.raises(InvalidPauseReasonError):
        _coerce_paused_reason("", phase_name="setup")
