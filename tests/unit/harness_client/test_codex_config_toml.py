...
def _server(
    ...
    *,
    ...
    return DesiredMcpServer(
        ...
        args=args,
        cwd="C:/proj",
        env=(("PROJECT_ID", "AG3"), ("AGENTKIT_CONCEPTS_DIR", "C:/proj/concepts")),
        required=True,
    )


def _render(raw: bytes | None, *servers: DesiredMcpServer) -> str:
    return render_codex_config(raw, hook_command=_HOOK, servers=servers)
...
def test_fresh_render_with_server_carries_all_five_fields_and_required() -> None:
    ...
    assert entry["env"]["PROJECT_ID"] == "AG3"
...
def test_canonical_render_is_deterministic_across_server_order() -> None:
    a = _server()
    b = _server(ARE_MCP_SERVER, command="agentkit-are-mcp", args=())
...
def test_merge_never_removes_an_ak3_server_written_earlier() -> None:
    ...
    raw = _render(None, _server(), _server(ARE_MCP_SERVER, command="x", args=())).encode(
        "utf-8"
    )
    text = _render(raw, _server())
    parsed = tomllib.loads(text)
...
def test_absent_file_is_foreign() -> None:
    assert classify_ownership(None, hook_command=_HOOK) is CodexConfigOwnership.FOREIGN


def test_hook_only_ak3_file_is_ak3_only() -> None:
    assert (
        classify_ownership(_hook_only(), hook_command=_HOOK)
...
def test_hook_plus_ak3_mcp_table_is_ak3_only() -> None:
    ...
    assert (
        classify_ownership(_hook_plus_mcp(), hook_command=_HOOK)
        is CodexConfigOwnership.AK3_ONLY
    )
...
def test_foreign_table_alongside_ak3_content_is_mixed() -> None:
    ...
    assert classify_ownership(raw, hook_command=_HOOK) is CodexConfigOwnership.MIXED
...
def test_different_hook_command_is_not_ak3_owned() -> None:
    ...
    assert classify_ownership(raw, hook_command=_HOOK) is CodexConfigOwnership.FOREIGN
...
def test_ak3_only_file_with_are_server_is_classified_owned() -> None:
    ...
    raw = _render(None, _server(ARE_MCP_SERVER, command="x", args=())).encode("utf-8")
    assert classify_ownership(raw, hook_command=_HOOK) is CodexConfigOwnership.AK3_ONLY
...
def test_render_canonical_matches_classification_for_every_owned_subset() -> None:
    ...
    for servers in ((), (_server(),), (_server(), _server(ARE_MCP_SERVER, command="x", args=()))):
        raw = _render(None, *servers).encode("utf-8")
        assert (
            classify_ownership(raw, hook_command=_HOOK)
            is CodexConfigOwnership.AK3_ONLY
        ), servers
...
def test_unknown_field_survives_the_merge_and_keeps_the_file_preserved() -> None:
    ...
    assert classify_ownership(merged, hook_command=_HOOK) is CodexConfigOwnership.MIXED
