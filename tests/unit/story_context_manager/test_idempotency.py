...
def test_compute_body_hash_excludes_op_id() -> None:
    ...
    body1 = {"project_key": "AK3", "title": "T1", "op_id": "op-aaa"}
    body2 = {"project_key": "AK3", "title": "T1", "op_id": "op-bbb"}

    assert compute_body_hash(body1) == compute_body_hash(body2)


def test_compute_body_hash_different_values_differ() -> None:
    body1 = {"project_key": "AK3", "title": "T1"}
    body2 = {"project_key": "AK3", "title": "T2"}

    assert compute_body_hash(body1) != compute_body_hash(body2)


def test_compute_body_hash_is_deterministic() -> None:
    body = {"b": 2, "a": 1, "op_id": "x"}
...
def test_check_returns_false_for_new_op_id() -> None:
    store = _make_store()
    body = {"project_key": "AK3", "title": "New story"}
...
def test_check_returns_true_after_record() -> None:
    store = _make_store()
    body = {"project_key": "AK3", "title": "Story"}
    result_payload = {"story_id": "AK3-1", "status": "Backlog"}
...
def test_check_raises_on_body_hash_mismatch() -> None:
    ...
    store = _make_store()
    original_body = {"project_key": "AK3", "title": "Original"}
    different_body = {"project_key": "AK3", "title": "Different"}
