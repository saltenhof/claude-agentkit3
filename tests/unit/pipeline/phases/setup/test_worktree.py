...
from agentkit.pipeline.phases.setup.worktree import (
    RepoNotFoundError,
    setup_worktree,
    setup_worktrees,
)
...
def test_setup_worktrees_creates_one_worktree_per_participating_repo(
    ...
    with (
        patch(
            "agentkit.pipeline.phases.setup.worktree.branch_exists",
            return_value=False,
        ),
        patch("agentkit.pipeline.phases.setup.worktree.create_worktree") as create,
    ):
...
def test_setup_worktrees_fails_for_unknown_participating_repo(
    ...
    with (
        ...
        patch("agentkit.pipeline.phases.setup.worktree.create_worktree"),
        patch("agentkit.pipeline.phases.setup.worktree.remove_worktree") as remove,
...
def test_setup_worktrees_cleans_up_previous_repos_on_later_failure(
    ...
    with (
        ...
        patch(
            "agentkit.pipeline.phases.setup.worktree.create_worktree",
