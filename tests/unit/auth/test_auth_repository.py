...
def test_revoked_project_api_token_id_cannot_be_registered_again(tmp_path: Path) -> None:
    project_repo = StateBackendProjectRepository(tmp_path)
    project_repo.save(
        Project(
            key="tenant-a",
    ...
    repository.revoke("tenant-a", original.token_id)

    with pytest.raises(ProjectApiTokenAlreadyExistsError):
        repository.insert(
            original.model_copy(
                update={"token_hash": "attacker-hash", "revoked_at": None},
            ),
        )

    persisted = repository.get(original.token_id)
    assert persisted is not None
    assert persisted.token_hash == "original-hash"
