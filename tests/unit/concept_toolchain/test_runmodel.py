...
def test_valid_run_state_loads(tmp_path: Path) -> None:
    run, issues = runmodel_run.load_run_state(write_payload(tmp_path / "RUN.json", valid_run_payload()))
    assert issues == []
...
def test_unknown_run_field_is_fail_closed(tmp_path: Path) -> None:
    ...
    run, issues = runmodel_run.load_run_state(write_payload(tmp_path / "RUN.json", payload))
    assert run is None
...
def test_valid_lease_loads(tmp_path: Path) -> None:
    ...
    lease, issues = runmodel_run.load_lease(write_payload(tmp_path / "LEASE.json", payload))
    assert issues == []
...
def test_round_outcome_reason_required_unless_received(tmp_path: Path) -> None:
    ...
    state, issues = runmodel_run.load_round_state(write_payload(tmp_path / "ROUND.json", payload))
...
def test_tsv_header_contract_is_exact(tmp_path: Path) -> None:
    rows, issues = runmodel_registers.load_source_units(write_tsv_text(tmp_path / "u.tsv", "unit_id\tWRONG\n"))
    assert rows == ()
...
def test_tsv_rows_must_be_strictly_sorted(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_source_units(write_tsv_text(tmp_path / "u.tsv", text))
    assert "strictly sorted" in messages(issues)


def test_tsv_wrong_column_count_rejected(tmp_path: Path) -> None:
    _, issues = runmodel_registers.load_source_units(
        write_tsv_text(tmp_path / "u.tsv", f"{UNITS_HEADER}\nSU-ab12cd34-0001\tonly\n")
    )
...
def test_unit_row_requires_claims_or_reason_in_checker_mode(tmp_path: Path) -> None:
    text = f"{UNITS_HEADER}\n{UNIT_ROW}\t\t\n"
    _, strict_issues = runmodel_registers.load_source_units(write_tsv_text(tmp_path / "u.tsv", text))
    assert "claim_refs or an empty_reason" in messages(strict_issues)
    _, lenient_issues = runmodel_registers.load_source_units(tmp_path / "u.tsv", require_disposition=False)
...
def test_ledger_rejected_requires_reason(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_disposition_ledger(write_tsv_text(tmp_path / "l.tsv", text))
...
def test_atom_covered_requires_receipts_and_targets(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_atom_register(write_tsv_text(tmp_path / "a.tsv", text))
...
def test_atom_covered_split_needs_two_targets(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_atom_register(write_tsv_text(tmp_path / "a.tsv", f"{header}\n{row}\n"))
...
def test_source_coverage_na_requires_reason(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_source_coverage(write_tsv_text(tmp_path / "c.tsv", text))
...
def test_findings_resolution_required_when_resolved(tmp_path: Path) -> None:
    ...
    _, issues = runmodel_registers.load_findings_register(write_tsv_text(tmp_path / "f.tsv", text))
...
def test_canonical_request_digest_ignores_request_digest_field() -> None:
    ...
    digest = runmodel_digests.canonical_request_digest(payload)
    assert digest == runmodel_digests.canonical_request_digest(payload | {"request_digest": SHA})
    assert digest != runmodel_digests.canonical_request_digest(payload | {"gate": "scope-consistency"})
...
def test_canonical_projection_entry_digest_strips_derived_fields() -> None:
    ...
    digest = runmodel_digests.canonical_projection_entry_digest(entry, "concept/_meta/projection-manifest.json")
    ...
    assert digest == runmodel_digests.canonical_projection_entry_digest(mutated, "concept/_meta/projection-manifest.json")
...
def test_scope_lock_filename_normalization() -> None:
    name = runmodel_digests.scope_lock_filename("Sample_Scope.two")
...
def test_semantic_pack_request_digest_is_verified(tmp_path: Path) -> None:
    ...
    payload["request_digest"] = runmodel_digests.canonical_request_digest(payload)
    pack, issues = runmodel_semantic.load_semantic_request_pack(write_payload(tmp_path / "p.json", payload))
    assert issues == []
...
def test_projection_manifest_accepts_optional_covered_scope_ids(tmp_path: Path) -> None:
    ...
    manifest, issues = runmodel_projection.load_projection_manifest(write_payload(tmp_path / "m.json", payload))
    assert issues == []
...
def test_canonical_tsv_subset_digest_equals_file_digest_at_freeze(tmp_path: Path) -> None:
    ...
    assert runmodel_digests.canonical_tsv_subset_digest(header, rows) == runmodel_digests.file_sha256(path)
    assert runmodel_digests.canonical_tsv_subset_digest(header, list(reversed(rows))) == runmodel_digests.file_sha256(path)
...
def test_derive_register_digests_partitions_input_and_derived(tmp_path: Path) -> None:
    ...
    register_header = runmodel_pins.SOURCE_REGISTER_HEADER
    ...
    units_header = runmodel_pins.SOURCE_UNITS_HEADER
    ...
    claims_header = runmodel_pins.CLAIMS_INVENTORY_HEADER
    ...
    derived, issues = runmodel_pins.derive_register_digests(run_dir)
    assert issues == []
    assert derived["source_register_input"] == runmodel_digests.canonical_tsv_subset_digest(register_header, [input_row])
    assert derived["source_register_final"] == runmodel_digests.file_sha256(register)
    assert derived["source_units_input"] == runmodel_digests.canonical_tsv_subset_digest(units_header, [unit_input])
    assert derived["claims_inventory_input"] == runmodel_digests.canonical_tsv_subset_digest(claims_header, [])
    assert derived["derived_claims"] == runmodel_digests.canonical_tsv_subset_digest(claims_header, [claim_derived])
...
def test_registry_edges_accept_string_and_object_forms(tmp_path: Path) -> None:
    ...
    manifest, issues = runmodel_promotion.load_promotion_manifest(write_payload(tmp_path / "m.json", payload))
    assert issues == []
    assert manifest is not None
    assert manifest.required_registry_edges[0] == "a.yaml#anchor"
    assert isinstance(manifest.required_registry_edges[1], runmodel_promotion.RegistryEdge)
...
def test_lock_evidence_loader_is_fail_closed(tmp_path: Path) -> None:
    ...
    evidence, issues = runmodel_locks.load_lock_evidence(write_payload(tmp_path / "e.json", payload))
    assert issues == []
...
def test_timestamp_expired() -> None:
    assert runmodel_digests.timestamp_expired("2020-01-01T00:00:00Z", 60) is True
    assert runmodel_digests.timestamp_expired("2999-01-01T00:00:00Z", 60) is False
