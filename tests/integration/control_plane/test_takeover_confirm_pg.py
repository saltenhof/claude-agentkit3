...
def test_takeover_confirm_global_commits_all_side_effects_atomically(
    ...
    commit_takeover_confirm_global(
        ...
        events=(
            ExecutionEventRecord(
                ...
                event_type=EventType.RUN_OWNERSHIP_TAKEOVER_CONFIRMED.value,
                occurred_at=_NOW,
                source_component="project_edge_client",
                severity="info",
                phase="ownership",
                payload={"new_owner_session_id": "sess-B"},
            ),
        ),
    )
    ...
    assert [event.event_type for event in events] == [
        EventType.RUN_OWNERSHIP_TAKEOVER_CONFIRMED.value
    ]


def _op_record(new_binding: SessionRunBindingRecord) -> ControlPlaneOperationRecord:
