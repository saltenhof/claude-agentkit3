...
# ---------------------------------------------------------------------------
# AC5: queue-fairness (pending_project_claims_are_not_overtaken_by_younger_story_claims)
# ---------------------------------------------------------------------------


def test_project_claim_blocks_while_a_story_claim_is_held() -> None:
    identity = boot_backend_instance_identity_global("inst-fairness-1", _T0)
    ...
    #: A project-wide mutation cannot proceed while the story object is held.
    assert (
        acquire_object_mutation_claim_global(
            project_key=_PROJECT,
            serialization_scope=oc.PROJECT_SCOPE,
            scope_key=_PROJECT,
            op_id="op-project",
            backend_instance_id=identity.backend_instance_id,
            instance_incarnation=identity.instance_incarnation,
    ...
    assert delete_object_mutation_claim_global(
        _PROJECT, oc.STORY_SCOPE, "AG3-600", "op-story"
    )
    #: Now free -- the project claim succeeds.
    assert acquire_object_mutation_claim_global(
        project_key=_PROJECT,
        serialization_scope=oc.PROJECT_SCOPE,
        scope_key=_PROJECT,
...
def test_younger_story_claim_never_overtakes_a_held_project_claim() -> None:
    """AC5 crux: once a project claim is held, a LATER-arriving story claim of
    the SAME project must fail-closed -- it never overtakes.
    """
    identity = boot_backend_instance_identity_global("inst-fairness-2", _T0)
    ...
    #: A YOUNGER (later-arriving) story claim of the SAME project must lose,
    #: even though its own object (a different scope_key) never PK-collides.
    assert (
        acquire_object_mutation_claim_global(
            project_key=_PROJECT,
            serialization_scope=oc.STORY_SCOPE,
            scope_key="AG3-601",
            op_id="op-younger-story",
            backend_instance_id=identity.backend_instance_id,
            instance_incarnation=identity.instance_incarnation,
            acquired_at=_T0 + timedelta(seconds=5),
        )
        is False
    )


def test_lock_set_acquires_project_then_stories_against_real_postgres() -> None:
    """End-to-end smoke test: ``object_claims.acquire_lock_set`` over the REAL
    Postgres port acquires a multi-object lock-set in canonical order.
    """
    from agentkit.backend.control_plane.repository import ObjectMutationClaimRepository

    identity = boot_backend_instance_identity_global("inst-lockset", _T0)
    port = ObjectMutationClaimRepository()
    lock_set = oc.build_lock_set(
        _PROJECT, story_ids=("AG3-610", "AG3-611"), include_project_claim=True
    )
    ...
    assert load_object_mutation_claim_global(_PROJECT, "project", _PROJECT) is not None
    assert load_object_mutation_claim_global(_PROJECT, "story", "AG3-610") is not None
    assert load_object_mutation_claim_global(_PROJECT, "story", "AG3-611") is not None
    oc.release_lock_set(port, lock_set, op_id="op-lockset")
    assert load_object_mutation_claim_global(_PROJECT, "project", _PROJECT) is None
